...
namespace XorShiftAddSharpTest
{
	...
	{
		public InternalStateTest(ITestOutputHelper output)
		{
			_output = output;
		}
		...
		public void CopyToTest()
		{
			...
			for (int i = 0; i < InternalState.Size; i++) scr[i] = (uint) (i + 10);
			...
			for (int i = 0; i < InternalState.Size; i++) actual[i].Is(scr[i]);
		}
		...
		public unsafe void IndexerGetTest()
		{
			...
			for (uint i = 0; i < InternalState.Size; i++) actual.State[i] = i + 10u;

			for (int i = 0; i < InternalState.Size; i++) actual[i].Is(actual.State[i]);
		}
