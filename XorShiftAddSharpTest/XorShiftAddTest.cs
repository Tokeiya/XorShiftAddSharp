...
namespace XorShiftAddSharpTest
{
	...
	{
		public XorShiftAddTest(ITestOutputHelper output)
		{
			_output = output;
		}
		...
		private static unsafe void AssertInternalVector(in InternalState expected, XorShiftAdd actual)
		{
			var state = actual.GetCurrentState();
			for (int i = 0; i < InternalState.Size; i++) expected.State[i].Is(state[i]);
		}
		...
		public void CalculateJumpPolynomialTest()
		{
			static void assert(ReadOnlySpan<char> expected, string actual)
			{
				...
				for (int j = 0; j < i; j++) expected[j].Is(actual[j]);
			}
		...
		public unsafe void JumpTestA()
		{
			static void test(string bs)
			{
				...
				for (int i = 0; i < InternalState.Size; i++) state[i].Is(expected.State[i]);
			}
		...
		public void NextBytesTestA()
		{
			...
			foreach (var i in actual) i.Is((byte) XorShiftAddCore.NextUint32(ref expected));
		}
		...
		public void NextDoubleTest()
		{
			...
			for (int i = 0; i < 100; i++) rnd.NextDouble().Is(XorShiftAddCore.NextDouble(ref expected));
		}
		...
		public void NextTestB()
		{
			...
			{
				var expected =
					(int) ((long) (XorShiftAddCore.NextDouble(ref state) * (maxValue - minValue)) + minValue);
		...
		public void RestoreTestA()
		{
			...
			for (int i = 0; i < InternalState.Size; i++) act.GetCurrentState()[i].Is(exp[i]);
		...
		public unsafe void TestNameB()
		{
			...
			for (var i = 0; i < InternalState.Size; ++i) actual.GetCurrentState()[i].Is(expected.State[i]);
